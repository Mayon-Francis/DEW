...
contract WarrantyNFT is ERC2771Recipient{
    ...
    modifier onlyCreator (uint _tokenId) {
        require(_msgSender() == idToWarranty[_tokenId].creator,
                "Sender is not creator of the warranty!");
        _;
    }

    modifier onlyApproved(uint _tokenId) {
        require(_msgSender() == idToApproved[_tokenId], 
    ...
    modifier onlyCreatorOrApproved(uint _tokenId) {
        require(_msgSender() == idToWarranty[_tokenId].creator || isApprovedAddress(_msgSender(), _tokenId),
    ...
    function transferTo(address _to, uint _tokenId) public {
        ...
        if (_msgSender() == idToWarranty[_tokenId].creator || isApprovedAddress(_msgSender(), _tokenId)) {
    ...
    function _transfer(address _to, uint _tokenId) private onlyValidToken(_tokenId) {
        ...
        if(!isApprovedAddress(_msgSender(), _tokenId) && !(_msgSender() == idToWarranty[_tokenId].creator)) {
            for (uint i=0; i < ownerToIds[_msgSender()].length; i++) {
                if (ownerToIds[_msgSender()][i] == _tokenId) {
                    ownerToIds[_msgSender()][i] = ownerToIds[_msgSender()][ownerToIds[_msgSender()].length - 1];
